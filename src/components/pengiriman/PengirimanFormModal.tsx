...
import { PengirimanBarang, Barang, PengirimanSample } from '../../types';
...
interface PengirimanFormModalProps {
  ...
  onSavePengiriman: (pengiriman: PengirimanBarang, updatedBarangIds: string[]) => void;
}
...
export const PengirimanFormModal: React.FC<PengirimanFormModalProps> = ({
  ...
  const handleScanBarcode = (codeToScan: string) => {
    ...
    if (selectedBarangIds.includes(matched.barang_id)) {
      setScanFeedback({
        type: 'info',
        message: `Bal ${matched.no_bal} (${matched.barcode}) sudah tercentang dalam daftar kirim.`,
      });
      setBarcodeInput('');
      return;
    }
  ...
  return (
      ...
      <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-3 sm:p-4 overflow-y-auto font-sans animate-in fade-in duration-150">
          ...
          <div className="px-5 py-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white">
            <div className="flex items-center space-x-2.5 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-gray-500 font-medium">
                  Pusat Pamekasan • Pilih atau scan barcode bal tembakau untuk muatan surat jalan
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2 shrink-0">
              ...
              <button
                ...
                className="px-4 py-1.5 text-xs font-bold text-white bg-[#b81d24] hover:bg-[#a0181e] disabled:opacity-50 disabled:cursor-not-allowed rounded-sm transition flex items-center space-x-1.5 cursor-pointer shadow-xs whitespace-nowrap"
              >
        ...
        <form onSubmit={handleSubmit} className="p-4 overflow-y-auto flex-1 space-y-4 text-xs">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 items-start">
            ...
            {/* Left: General Info & Armada (5 cols) */}
            <div className="lg:col-span-5 space-y-3">
              ...
              <div className="space-y-1">
                ...
                <select
                  value={tujuanPabrik}
                  onChange={(e) => {
                    setTujuanPabrik(e.target.value);
                    setCustomTujuan('');
                  }}
                ...
                >
                  ...
                  <option value="PT Gudang Garam Tbk Kediri - Gudang Produksi">PT Gudang Garam Tbk Kediri - Gudang Produksi</option>
                  <option value="PT HM Sampoerna Surabaya - Receiving Plant">PT HM Sampoerna Surabaya - Receiving Plant</option>
                  <option value="Bentoel Group Malang - Main Warehouse">Bentoel Group Malang - Main Warehouse</option>
                  <option value="Pabrik Rokok Sukun Kudus - Unit Rajang">Pabrik Rokok Sukun Kudus - Unit Rajang</option>
                  <option value="lainnya">-- Input Pabrik Tujuan Lainnya --</option>
                </select>
              </div>
              ...
              {tujuanPabrik === 'lainnya' && (
                <div className="space-y-1">
                  <label className="font-semibold text-gray-700 block">
                    Nama Pabrik Tujuan Baru <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                    required
                    ...
                    placeholder="Contoh: PT Wismilak Inti Makmur Surabaya"
                    className="w-full px-2.5 py-1.5 border border-[#ced4da] rounded-sm focus:border-[#b81d24] focus:outline-none bg-white text-gray-900"
                  />
                </div>
              )}

              {/* Tautkan Referensi Sample Approved */}
              <div className="space-y-1">
                <label className="font-semibold text-gray-700 block flex items-center space-x-1">
                  <LinkIcon className="w-3.5 h-3.5 text-[#b81d24]" />
                  <span>Tautkan Referensi Approval Sample</span>
                </label>
                <select
                  value={sampleRefId}
                  ...
                  className="w-full px-2.5 py-1.5 border border-[#ced4da] rounded-sm focus:border-[#b81d24] focus:outline-none bg-white text-gray-900"
                >
                  <option value="">-- Tanpa Referensi Sample Lab --</option>
                  {approvedSamples.map((s) => (
                    <option key={s.sample_id} value={s.sample_id}>
                      {s.sample_id} — Grade {s.kode_grade} (Lolos QC: {s.tujuan})
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                ...
                <div className="space-y-1">
                  <label className="font-semibold text-gray-700 block">
                    No. Polisi Truk <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                    required
                    ...
                    placeholder="AA 8923 BZ"
                    className="w-full px-2.5 py-1.5 font-mono font-bold border border-[#ced4da] rounded-sm focus:border-[#b81d24] focus:outline-none bg-white text-gray-900"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                <label className="font-semibold text-gray-700 block">
                  Catatan Tambahan Surat Jalan
                </label>
                <textarea
                  rows={2}
                  value={catatan}
                  onChange={(e) => setCatatan(e.target.value)}
                  placeholder="Kondisi terpal truk rapi, segel muatan utuh..."
                  className="w-full px-2.5 py-1.5 border border-[#ced4da] rounded-sm focus:border-[#b81d24] focus:outline-none bg-white text-gray-900"
                />
              </div>
              ...
              <div className="p-2.5 bg-white border border-gray-300 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-gray-900 flex items-center space-x-1.5">
                    ...
                    <span>TEMBAKKAN SCANNER GUN (OTOMATIS TERCENTANG)</span>
                  </span>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="Arahkan scanner / ketik misal: E-140826-034..."
                      ...
                      >
                        <div className="flex items-center space-x-2">
                          <input
                            ...
                            className="w-3.5 h-3.5 text-[#b81d24] rounded-none cursor-pointer"
                          />
                          <div>
    ...
    <ConfirmModal
      ...
      title="Konfirmasi Penerbitan Surat Jalan Pengiriman"
      message={`Apakah Anda yakin ingin menerbitkan Surat Jalan pengiriman untuk ${totalBalCount} bal tembakau?`}
      detail={`Tujuan: ${tujuanPabrik === 'lainnya' ? customTujuan : tujuanPabrik} | Total Tonase: ${totalBeratTonaseKg.toLocaleString('id-ID', { maximumFractionDigits: 1 })} Kg | Driver: ${driverNama} (${platNomor})`}
      variant="primary"
